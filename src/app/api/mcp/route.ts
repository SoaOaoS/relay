...
// Workspace root for file operations — sandboxed to this directory.
  ...
  try {
    switch (toolId) {
      ...
      case 'github-read': {
        ...
        const data = await res.json() as { content?: string; items?: unknown[]; total_count?: number }
        if (data.content) {
      ...
      case 'web-search': {
        ...
        if (ddgData.AbstractText) {
          results.push({ type: 'abstract', text: ddgData.AbstractText, source: ddgData.AbstractURL })
        }
      ...
      case 'web-fetch': {
        ...
        // Strip HTML tags crudely if content-type is HTML
      ...
      case 'file-read': {
        ...
        if (!safePath.startsWith(WORKSPACE_ROOT)) {
          return Response.json({ error: 'Path outside workspace' }, { status: 403 })
        }
      ...
      case 'file-write': {
        const { path: filePath, content: fileContent } = params as any
        const safePath = resolve(join(WORKSPACE_ROOT, filePath))
        if (!safePath.startsWith(WORKSPACE_ROOT)) {
          return Response.json({ error: 'Path outside workspace' }, { status: 403 })
        }
      ...
      case 'phone-call': {
        const { number, context } = params as any

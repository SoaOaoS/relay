...
// Client-side: uses @supabase/ssr so sessions sync to cookies automatically
export const supabase = createBrowserClient(SUPABASE_URL, SUPABASE_ANON_KEY)

// Server-side admin: uses service role key (server only, never exposed)
let _supabaseAdmin: SupabaseClient | null = null
export function getSupabaseAdmin() {
  if (!_supabaseAdmin) {
    _supabaseAdmin = createAdminClient(SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY || '')
  }
